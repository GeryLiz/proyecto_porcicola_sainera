...
<div class="container container-fluid">
  <form id="frmDeleteAll" action="<?php echo routing::getInstance()->getUrlWeb('departamento', 'deleteSelect') ?>" method="POST">
    ...
    <table class="table table-bordered table-responsive">
      ...
      <tbody>
        <?php foreach ($objDepartamento as $key): ?>
          <tr>
            <td><input type="checkbox" name="chk[]" value="<?php echo $key->id ?>"></td>
            <td><?php echo $key->id ?></td>
            <td>
                <?php echo $key->desc_departamento ?>
            </td>
            <td>
                <a href="<?php echo routing::getInstance()->getUrlWeb('departamento', 'edit', array(departamentoTableClass::ID => $key->id)) ?>" class="btn btn-primary btn-xs">Editar</a>
